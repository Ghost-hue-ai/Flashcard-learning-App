...
function PracticePage() {
    ...
    const completedSpanish = useSelector((state) => state.card?.completedSpanishCard || {});
    const dispatch = useDispatch();
    const { languageName } = useParams();

    const getRandomIndex = useCallback((dataLength, used) => {
        const available = Array.from({ length: dataLength }, (_, i) => i).filter(i => !used.includes(i));
        ...
        if(languageName === 'learnSpanish') {
            ...
            if (stored) {
                try {
                    ...
                    console.error("Failed to parse usedSpanishCard from localStorage:", err);
                }
            }
        }
                ...
                usedSpanishIndexes.length < spanishSentences.length ? (
                    <div className="w-full max-w-xl flex flex-col items-center gap-6">
                                    ...
                                    <FlashCard
                                        ...
                                        handleClick={({ id, question, answer }) => {
                                            setTimeout(() => {
                                                ...
                                                dispatch(updateRecentLogs(`Completed Spanish Learning Flashcard #${id}`));
                                            }, 4000);
                                        }}
                                    />
                                );
                        ...
                        <div className="flex bg-gray-700 w-full justify-between px-2 py-4 rounded-xl">
                            ...
                            <span className="font-bold text-2xl"> {usedSpanishIndexes.length}/{spanishSentences.length}</span>
                        </div>
                    </div>
                ) : (
                    ...
                    <div className="w-full max-w-xl flex flex-col items-center gap-6">
                        ...
                        <div className="w-full bg-gray-700 flex items-center justify-between p-4 rounded-3xl">
                            ...
                            <span className="text-sm text-slate-300 font-mono">
                                Progress: <span className="text-white font-bold">{usedEnglishIndexes.length}</span> / {englishDic.length}
                            </span>
                        </div>
                    </div>
                ) : (
